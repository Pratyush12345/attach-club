    ...
    return Consumer<ChangeSearchScreenProvider>(
      builder: (context, model, child) {
        if(context.read<ConnectionsBloc>().connectedList.indexWhere((element) => element.uid == userData.uid) ==-1){
         ...
         if(context.read<ConnectionsBloc>().receivedList.indexWhere((element) => element.uid == userData.uid)==-1){
           
           if(context.read<ConnectionsBloc>().sentList.indexWhere((element) => element.uid == userData.uid)==-1){
             buttonText =  "Connect";
           }
           else{
                            ...
                            child: ElevatedButton(
                              ...
                              onPressed: () {
                                if (buttonText == "Connect") {
                                  ...
                                  context.read<ConnectionsBloc>().sentList.add(ConnectionRequest.fromMap(connectionData: {"status" : CONNECTION_SENT_STATUS, 
                                   "updateTime" : Timestamp.now()
                                  }, uid: userData.uid!, userData: userData.toMap()) );
